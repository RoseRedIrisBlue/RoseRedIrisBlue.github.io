...
let BattleMovedex = {
	...
	"facade": {
		...
		onBasePower: function (basePower, pokemon) {
			if ((pokemon.status && pokemon.status !== 'slp') || pokemon.hasAbility('terminal')) {
				return this.chainModify(2);
			}
		},
		secondary: false,
	...
	"hex": {
		...
		basePower: 65,
		basePowerCallback: function (pokemon, target, move) {
			if (target.status || target.hasAbility('comatose') || target.hasAbility('terminal')) return move.basePower * 2;
			return move.basePower;
		},
		category: "Special",
	...
	"honeclaws": {
		...
		basePower: 0,
		category: "Status",
		desc: "Raises the user's Attack and accuracy by 1 stage; 2 in Nightfall.",
		shortDesc: "Raises the user's Attack and accuracy by 1; 2 in Nightfall.",
		id: "honeclaws",
		isViable: true,
		name: "Hone Claws",
		pp: 15,
		...
		flags: {snatch: 1},
		onModifyMove: function (move) {
			if (this.isWeather(['nightfall'])) move.boosts = {atk: 2, accuracy: 2};
		},
		boosts: {
			atk: 1,
			accuracy: 1,
	...
	"moonlight": {
		...
		flags: {snatch: 1, heal: 1},
		onHit: function (pokemon) {
			if (this.isWeather(['sunnyday', 'desolateland','nightfall'])) {
	...
	"morningsun": {
		...
		onHit: function (pokemon) {
			if (this.isWeather(['sunnyday', 'desolateland'])) {
				return this.heal(this.modify(pokemon.maxhp, 0.667));
			} else if (this.isWeather(['raindance', 'primordialsea', 'sandstorm', 'hail','nightfall'])) {
				return this.heal(this.modify(pokemon.maxhp, 0.25));
			} else {
				return this.heal(this.modify(pokemon.maxhp, 0.5));
			}
	...
	"phantomforce": {
		...
		onTry: function (attacker, defender, move) {
			...
			if (this.isWeather(['nightfall']) || !this.runEvent('ChargeMove', attacker, defender, move)) {
				this.add('-anim', attacker, move.name, defender);
				return;
			}
			attacker.addVolatile('twoturnmove', defender);
	...
	"venoshock": {
		...
		onBasePowerPriority: 4,
		onBasePower: function (basePower, pokemon, target) {
			if (target.status === 'psn' || target.status === 'tox' || target.hasAbility('terminal')) {
				return this.chainModify(2);
			}
		},
		secondary: false,
	...
	"nightfall": {
		...
		basePower: 0,
		category: "Status",
		desc: "For 5 turns, the weather becomes Nightfall. The damage of Ghost and Dark-type attacks is multiplied by 1.3 and the damage of Fairy-type attacks is multiplied by 0.75 during the effect. Lasts for 8 turns if the user is holding Dusk Ball. Fails if the current weather is Nightfall.",
