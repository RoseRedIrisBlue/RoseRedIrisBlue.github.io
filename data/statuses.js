...
let BattleStatuses = {
	...
	nightfall: {
		...
		duration: 5,
		durationCallback: function (source, effect) {
			if (source && source.hasItem('duskball')) {
				return 8;
			}
			return 5;
		},
		onWeatherModifyDamage: function (damage, attacker, defender, move) {
			if (move.type === 'Dark') {
				this.debug('Nightfall dark boost');
				return this.chainModify(1.3);
			}
			if (move.type === 'Ghost') {
				this.debug('Nightfall ghost boost');
				return this.chainModify(1.3);
			}
			if (move.type === 'Fairy') {
				this.debug('Nightfall fairy suppress');
		...
		onStart: function (battle, source, effect) {
			if (effect && effect.effectType === 'Ability') {
				if (this.gen <= 5) this.effectData.duration = 0;
				this.add('-weather', 'Nightfall', '[from] ability: ' + effect, '[of] ' + source);
			} else {
				this.add('-weather', 'Nightfall');
			}
		},
		onResidualOrder: 1,
		onResidual: function () {
			this.add('-weather', 'Nightfall', '[upkeep]');
			this.eachEvent('Weather');
		},
		onEnd: function () {
			this.add('-weather', 'none');
