...
let BattleAbilities = {
	...
	"eternight": {
		shortDesc: "On switch-in, this Pokemon summons Nightfall.",
		onStart: function (source) {
			this.setWeather('nightfall');
		},
		id: "eternight",
		name: "Eternight",
		rating: 4.5,
	...
	"shadowdance": {
		desc: "If Nightfall is active, this Pokemon's Speed is doubled.",
		shortDesc: "If Nightfall is active, this Pokemon's Speed is doubled.",
		onModifySpe: function (spe, pokemon) {
			if (this.isWeather('nightfall')) {
				return this.chainModify(2);
			}
		},
		id: "shadowdance",
	...
	"lunarpower": {
		desc: "If Nightfall is active, this Pokemon's Special Attack is multiplied by 1.5 and it loses 1/8 of its maximum HP, rounded down, at the end of each turn.",
		shortDesc: "If Nightfall is active, this Pokemon's Sp. Atk is 1.5x; loses 1/8 max HP per turn.",
		onModifySpAPriority: 5,
		onModifySpA: function (spa, pokemon) {
			if (this.isWeather(['nightfall'])) {
				return this.chainModify(1.5);
	...
	"nightlight": {
		desc: "If Nightfall is active, this Pokemon's Fire attacks are multiplied by 1.3. Healed by fire.",
		shortDesc: "If Nightfall is active, this Pokemon's fire attacks are 1.3x. Fire heals.",
		onTryHit: function (target, source, move) {
			if (target !== source && move.type === 'Fire') {
				if (!this.heal(target.maxhp / 4)) {
					this.add('-immune', target, '[msg]', '[from] ability: Night Light');
				}
				return null;
			}
		},
	...
	"clairvoyance": {
		desc: "This Pokemon's moves ignore substitutes, opposing side's Reflect, Light Screen, Safeguard, Mist and Aurora Veil, and bypasses protection.",
		shortDesc: "Moves ignore substitutes, screens/veil, protection.",
		onModifyMove: function (move) {
			move.infiltrates = true;
	...
	"terminal": {
		desc: "This Pokemon cannot be statused, and is considered to be poisoned. Moongeist Beam, Sunsteel Strike, and the Abilities Mold Breaker, Teravolt, and Turboblaze cannot ignore this Ability.",
		shortDesc: "This Pokemon cannot be statused, and is considered to be poisoned.",
		onStart: function (pokemon) {
			this.add('-ability', pokemon, 'Terminal');
		},
		onSetStatus: function (status, target, source, effect) {
			if (!effect || !effect.status) return false;
			this.add('-immune', target, '[msg]', '[from] ability: Terminal');
			return false;
		},
		// Permanent poisoned "status" implemented in the relevant poison-checking effects
		isUnbreakable: true,
		id: "terminal",
		name: "Terminal",
		rating: 3,
	...
	"prismguard": {
		desc: "Pokemon not making contact with this Pokemon lose 1/16 of their maximum HP, rounded down.",
		shortDesc: "Pokemon not making contact with this Pokemon lose 1/16 of their max HP.",
		onAfterDamageOrder: 1,
		onAfterDamage: function (damage, target, source, move) {
			if (source && source !== target && move && !(move.flags['contact'])) {
	...
	"ensnare": {
		desc: "This Pokemon is immune to Bug-type moves and restores 1/4 of its maximum HP, rounded down, when hit by an Bug-type move.",
		shortDesc: "This Pokemon heals 1/4 of its max HP when hit by Bug moves; Bug immunity.",
		onTryHit: function (target, source, move) {
			if (target !== source && move.type === 'Bug') {
				if (!this.heal(target.maxhp / 4)) {
					this.add('-immune', target, '[msg]', '[from] ability: Ensnare');
				}
				return null;
			}
		},
	...
	"riotshield": {
		desc: "This Pokemon is immune to ballistic moves, takes half damage from fire type moves, and takes reduced burn damage. Ballistic moves include Bullet Seed, Octazooka, Barrage, Rock Wrecker, Zap Cannon, Acid Spray, Aura Sphere, Focus Blast, and all moves with Ball or Bomb in their name.",
		shortDesc: "Makes user immune to ballistic moves, reduced fire and burn damage.",
		onBasePowerPriority: 7,
		onSourceBasePower: function (basePower, attacker, defender, move) {
			if (move.type === 'Fire') {
				return this.chainModify(0.5);
		...
		onTryHit: function (pokemon, target, move) {
			if (move.flags['bullet']) {
				this.add('-immune', pokemon, '[msg]', '[from] ability: Riot Shield');
				return null;
			}
		},
		id: "riotshield",
